...
Headless CLI proof-of-concept that integrates MoneyGram's cash-to-USDC on-ramp and USDC-to-cash off-ramp using Crossmint Smart Wallets on Stellar. Built with Deno 2.x.

## Architecture

This POC uses the **Bridge Account Pattern** (Option B). A local Ed25519 keypair acts as an intermediary between MoneyGram's anchor and the Crossmint smart wallet. See [docs/ARCHITECTURE_DECISION.md](docs/ARCHITECTURE_DECISION.md) for the full rationale.
...
Generate deterministic keypairs for the bridge account and client domain signing:
...
Set `BRIDGE_SEED` and `CLIENT_SIGNING_SEED` in `.env` to the seed strings you chose. Put the public key from the client domain keypair into your TOML server's `SIGNING_KEY`.
...
Fund the bridge account on testnet (via Friendbot) and establish a USDC trustline.
...
Initiate a USDC-to-cash withdrawal via SEP-24. Opens an interactive URL for KYC. When the anchor is ready, the CLI sends USDC from the bridge account to the anchor.
...
Display balances for both the bridge account (on-chain) and the Crossmint smart wallet.
...
This project uses deterministic keypair derivation instead of storing private keys. Given a seed string, the process is:

1. The seed string is hashed with SHA-256 using the Web Crypto API
2. The 32-byte hash is used as a raw Ed25519 seed via `Keypair.fromRawEd25519Seed()`
3. The same seed always produces the same G.../S... keypair

This means you only need to store seed strings in your `.env` file. The keypairs are derived at runtime. Use `deno task generate-keys --seed <seed>` to preview the public key for any seed.

The implementation is in `src/keys.ts`.

## TOML Server

The TOML server is a Hono app deployed on Vercel that serves `/.well-known/stellar.toml`. This is required for SEP-10 client_domain verification.
...
The server is configured via `toml-server/vercel.json`. Deploy with the Vercel CLI or connect the repository to Vercel.
